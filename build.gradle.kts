...
val desktopDir = file("${System.getProperty("user.home")}/Desktop/NotaSpese_v1.3.7")
tasks.register("buildAndCopyAll") {
    ...
    doLast {
        ...
        copy {
            ...
            into(desktopDir)
            rename { "NotaSpese_1.3.7.apk" }
        }
        copy {
            from(project(":desktop").file("build/compose/jars/NotaSpese-windows-x64-1.3.7.jar"))
            into(desktopDir)
        }
        // Copia exe se disponibile (chiudere l'app prima di eseguire :desktop:createDistributable)
        val exeDir = project(":desktop").file("build/compose/binaries/main/app/NotaSpese")
        if (exeDir.exists()) {
            copy { from(exeDir); into(desktopDir) }
        } else {
            println("Nota: eseguire ':desktop:createDistributable' (con app chiusa) per generare NotaSpese.exe")
        }
        project.file("$desktopDir/Avvia NotaSpese.bat").writeText("""
            @echo off
            cd /d "%~dp0"
            if exist "NotaSpese.exe" ( start "" "NotaSpese.exe" )
            else ( java -jar "NotaSpese-windows-x64-1.3.7.jar" )
        """.trimIndent())
