...
object NotaSpeseImporter {
    ...
    fun importFromFile(notaspeseFile: File): NotaSpeseConSpese? {
        if (!notaspeseFile.exists() || !notaspeseFile.name.endsWith(".notaspese", ignoreCase = true)) return null
        return try {
            val extractDir = File(System.getProperty("java.io.tmpdir"), "notaspese_${System.currentTimeMillis()}")
            extractDir.mkdirs()
            try {
                extractZip(notaspeseFile, extractDir)
